...
To specifically examine the transition states following runner on second and no outs, Table 7 was created to filter for only the transition proportions following that state. Looking at this table it can be seen that the next most likely state following a runner on second and no outs is a runner on second with one out (010 1) at 0.38. This state is followed in likelihood by a runner on third with one out (001 1) at 0.27. However, this table only shows the next most likely base-out state after one at-bat. Alternatively, Table 8 shows the the likelihood of new base-out states after two at-bats, from which it can be seen that the most likely base-out state is runner on third and two outs (001 2) at 0.174. This is additionally followed in likelihood by runner on second and two outs (010 2) at 0.168. Lastly, Table 89shows the likelihood of base-out states after three at-bats from the beginning state of runner on second and no outs. In this table it can be seen that the most likely state is three outs at 0.33. This is followed in likelihood by runner on first and two outs at 0.12.
...
As a final analysis a multi-chain analysis was performed to simulate the number of runs scored in a half-inning across all seasons. For this analysis, runs scored was simulated using the formula $RUNS = (N^{(b)}_{runners} + O^{(b)} + 1) - (N^{(a)}_{runners} + o^{(a)})$, where runs scored $RUNS$ is equal to difference between the sum of runners $N_{runners}$ and outs $O$ before $(b)$ the event plus one ($1$) and the number of runners $N_{runners}$ plus outs $O$ after $(a)$ after the event (Marchi et al., 2019b). The multi-chain analysis was computed by computing the number of runs for a given state using the given formula and then samples of size one (1) were selected for the length of a vector of length 25 using the probability weights from the given transition matrix. Runs were then returned as an indexed location from the calculated runs as weighted by the probabilities provided by the transition matrix. The simulation was then set to run for 20,000 simulations, results of which can be seen in Table 10. From this table it can be seen that the most likely outcome in a half-inning is that no runs are scored at 0.73. Additionally, the probability of scoring only one run can be seen as 0.14, whereas the probability of scoring one or more runs can be seen as 0.27. Lastly, the probability of scoring two or more runs in a half-inning is 0.13, slightly less than the probability of scoring only one run at 0.14.
