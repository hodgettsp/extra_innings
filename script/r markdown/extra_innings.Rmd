...
```{r parse retrosheet, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
...
for(i in 1:length(t2)){
  ...
  assign(paste0(t2[i], "_prob"), dplyr::filter(get(paste0(t2[i], "_roi")), outs_inning == 3) %>%
...
for(i in 1:length(t2)){
  ...
  assign(paste0(t2[i], "_runsout"), as.matrix(round(get(paste0(t2[i], "_runs"))[2], 2)))
  # assign object under previously used name as a matrix of dimensions 8 rows, 3 columns
  assign(paste0(t2[i], "_runsout"), matrix(get(paste0(t2[i], "_runsout")), 8, 3))
  ...
  assign(paste0(t2[i], "_runsout"), magrittr::set_colnames(get(paste0(t2[i], "_runsout")), 
...
for(i in 1:length(t3)){
  assign(paste0(t2[i], "_tmatrix"), dplyr::select(get(t4[i]), 
                                                 state, 
                                                 new_state) %>% 
           table())
}
...
for(i in 1:length(t2)){
  assign(paste0(t2[i], "_2Bpmatrix"), get(paste0(t2[i], "_pmatrix"))[7, ])
  assign(paste0(t2[i], "_2Bpmatrix"), as.matrix(get(paste0(t2[i], "_2Bpmatrix"))))
  assign(paste0(t2[i], "_2Bpmatrix"), get(paste0(t2[i], "_2Bpmatrix"))[-25, ])
  assign(paste0(t2[i], "_2Bpmatrix"), cbind(get(paste0(t2[i], "_prob")), get(paste0(t2[i], "_2Bpmatrix"))))
  assign(paste0(t2[i], "_2Bpmatrix"), dplyr::rename(get(paste0(t2[i], "_2Bpmatrix")), transfer_prob = 3))
  assign(paste0(t2[i], "_2Bpmatrix"), dplyr::mutate(get(paste0(t2[i], "_2Bpmatrix")),
                                                    combined_prob = case_when(transfer_prob > 0 ~ (prob * prob[7]),
                                   TRUE ~ prob * transfer_prob)))
}
...
for(i in 1:length(t3)){
  assign(paste0(t2[i], "_2Btmatrix"), dplyr::select(get(t4[i]), 
                                                 state, 
                                                 new_state) %>%
...
# the probability table matrix
for(i in 1:length(t3)){
  assign(paste0(t2[i], "_2Bpmatrix"), prop.table(get(t5[i]), 1))
  assign(paste0(t2[i], "_2Bpmatrix"), rbind(get(paste0(t2[i], "_2Bpmatrix")),
                                          c(rep(0, 5), 1)))
}

x <- as.data.frame(df2019_tmatrix)
y <- as.data.frame(df2019_2Btmatrix)
...
for(i in 1:length(t3)){
  assign(paste0(t2[i], "_rout"), sapply(row.names(get(t5[1])),
...
r_simulation <- replicate(10000, sim_half_inning(df2019_tmatrix, df2019_rmatrix))
