...
ggplot(combined_counts, aes(x = winning_team, y = count, fill = winning_team))+
  ...
  # set colour values
  scale_fill_manual(values = c("#FEC6D6", "#F7E775"),
...
for(i in 1:length(t2)){
  ...
  assign(paste0(t2[i], "_prob"), dplyr::filter(get(paste0(t2[i], "_roi")), outs_inning == 3) %>%
           ...
           dplyr::summarise(prob = mean(runs_roi >= 1)))
}
...
for(i in 1:length(t3)){
  assign(paste0(t2[i], "_rout"), sapply(row.names(get(t3[1])),
...
r_simulation <- replicate(10000, sim_half_inning(df2020_tmatrix, df2020_rmatrix))
