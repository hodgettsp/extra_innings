...
ggplot(combined_counts, aes(x = winning_team, y = count, fill = winning_team))+
  ...
  # set colour values
  scale_fill_manual(values = c("#00a9e0", "#f5003d"),
...
df_dblstate <- dftotal_joined %>% 
  mutate(dbl_state = case_when(state == "010 0" ~ new_state)) %>% 
  filter(state %in% dbl_state)

tmatrix_dblstate <- df_dblstate %>% 
  select(state, new_state) %>% 
  table()

pmatrix_dblstate <- round(prop.table(tmatrix_dblstate, 1), 4)
pmatrix_dblstate <- rbind(pmatrix_dblstate, c(rep(0, 12), 1))
...
kableExtra::kbl(pmatrix_total, caption = "Proportional Matrix for Transitions Between Base-Out States") %>% 
  kableExtra::kable_styling(position = "center", latex_options = c("HOLD_position", "striped", "scaled_down"))
```

To specifically examine the transition states following runner on second and no outs, Table 6 was created to filter for only the transition proportions following that state. Looking at this table it can be seen that the next most likely state following a runner on second and no outs is a runner on second with one out (010 1) at 0.38. This state is followed in likelihood by a runner on third with one out (001 1) at 0.27. However, this table only shows the next most likely base-out state after one at-bat. Alternatively, Table 7 shows the the likelihood of new base-out states after two at-bats, from which it can be seen that the most likely base-out state is runner on third and two outs (001 2) at 0.174. This is additionally followed in likelihood by runner on second and two outs (010 2) at 0.168. Lastly, Table 8 shows the likelihood of base-out states after three at-bats from the beginning state of runner on second and no outs. In this table it can be seen that the most likely state is three outs at 0.33. This is followed in likelihood by runner on first and two outs at 0.12.
...
As a final analysis a multi-chain analysis was performed to simulate the number of runs scored in a half-inning
...
########################################################
runners_out_dbl <- sapply(row.names(tmatrix_total),
                      count_runners_out)[-20]

# assign new object rtotal to calculate runs scored formula
rdbl <- outer(runners_out_dbl + 1, runners_out_dbl, FUN = "-")
# assign names in rtotal using names in tmatrix
names(rdbl) <- names(tmatrix_total)[-20]
# bind final column of 0s to rtotal
rdbl <- cbind(rdbl, rep(0, 19))
```
...
sim_half_inning <- function(.tmatrix, .rmatrix, start = 1){
  ...
  while(s < 20){
    ...
    s_new <- sample(1:20, size = 1, prob = .tmatrix[s, ])
...
runs_simulation <- replicate(20000, sim_half_inning(tmatrix_dblstate, rdbl))
