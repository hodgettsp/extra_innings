---
title: "A Consideration of Game Length and Probability of Runs Scored with Major League Baseball Runner on Second Extra-Inning Rule"
author: "Paul A. Hodgetts"
date: "16/03/2021"
output: 
  pdf_document:
    latex_engine: lualatex
abstract: "In the COVID-19 shortened Major League Baseball season of 2020, the MLB head-office implemented a rule that stated if a game were to go beyond the nine regulation innings, then each team would begin each extra half-inning with a runner on second base. This rule was implemented in hopes that it would limit exposure between players by shortening the length of extra-inning games by increasing the probability of a run being scored. This paper explores whether the rule did indeed shorten the length of extra-inning games and subsequently protect players, as well as whether the rule provided an advantage to the visitor team. Results of a Bayesian t-test suggest that the rule may have shortened the length of extra-inning games and subsequently protected players. Additionally, it is discussed that any advantage as provided by the rule may only be an exaggeration of existing structures within extra-inning games, and that beneficial results of the extra-inning rule may be a result of a limited sample size. Lastly, extra-inning strategy, probability of run scoring, and limitations are discussed."
---
...
Extra-inning games are played such that the game ends within a full inning when either the home team scores more runs than the visitor team, or the visitor scores more runs and prevents the home team from scoring. Thus, if the visitor team scores in their half of the inning, it becomes more difficult for the home team to win as they now need to score at least one more run than the visitor. As such, the strategy with which a manager approaches extra-inning games may differ from how they would the regulation innings. Especially with the implementation of the runner-on-second rule. For instance, by beginning the half-inning with a runner already on base, the visitor team need not play to get a runner on base and raise their subsequent probability of winning. Indeed, by only needing one run and then to prevent the home team from scoring the visitor team can sacrifice batters to move the runner along. For example, a sacrifice bunt to move the runner to third. Which despite having a lower number of average runs scored than a runner on second with no outs (1.14 to 0.97, see Table 4) does increase the probability of at least one run scoring from 0.62 to 0.66 (see Table 6). This differs from the standard way in which an extra-innings game begins, as in that state of no runners and no outs a manager would strategize to get players immediately on base so as to increase the probability of scoring a run from 0.28 (see Table 6). As such, the rule of runner-on-second may change the way in which extra-inning games are approached. 

## Advantage

However, given that the most likely results after three at-bats with a runner on second with no outs is three outs, it cannot be concluded that the runner-on-second provides an advantage to the visitor team. At least, not anymore than previously existed within the standard format of extra-inning games. Additionally, that extra-inning games seem to follow the pattern of more home team wins, this advantage may not be so great as to produce a discernible effect.

## Player Safety

Given the results of the Bayesian t-test analysis, it may be suggested that the run-on-second rule was successful in shortening the length of extra-inning games and the exposure players experienced between teams. As such, MLB may ahve been correct in implementing the rule to ensure the safety of their players. However, given that the 2020 season only amounted to 68 observations in extra-inning games, this finding may change if the rule were implemented within a longer season.

# Conclusion

In conclusion, the runner-on-second rule for extra-inning games may indeed protect players from an increased exposure to players on other teams. Thereby protecting the players from possible exposure to COVID-19 and protecting the health of the players and organization staff. Given that COVID-19 can lead to death and unknown future health concerns, this rule is acting within the best interests of the players' health outside of allowing ties after nine innings or not hosting a season. However, these beneficial results may only be caused by the shortened season, and a longer season may not show a similar effect. Second, this rule does not seem to make the playing-field any less fair than was already the case regarding extra-inning games. While the rule may change the way in which managers approach extra-inning games, the most likely outcome for the given base-out state is still three outs. As such, the rule only seems to exaggerate any effects that may already be present in the structure of extra-inning games. If further or more robust analysis does reveal that the rule does shorten the length of extra-inning games and MLB wishes to shorten the amount of baseball being played, then this may be a rule that MLB would look to implement long-term. However, that the rule does not necessarily improve upon the design of extra-innings, and may limit the amount of baseball played, this may not be a rule that MLB would be interested in implementing outside of the 2020 season.

# Limitations

The first limitation of this analysis is the limited sample size of games in which the runner-on-second rule was implemented. Given the limited sample size as compared to other seasons, it is difficult to make a conclusion from any comparison. Additionally, a more robust analysis than a Bayesian t-test may provide more conclusive results. Regarding the multi-chain model, this in general uses transitions from half-innings that begin with no runners on and no outs. A more robust analysis would use the priors from the transitions of states after a runner on second and no outs.
...
Douglas, C. & Scriven, R. (2020). retrosheet: Import Professional Baseball Data from 'Retrosheet'. R package version 1.1.3. <https://CRAN.R-project.org/package=retrosheet>
