...
##----
# THIS SECTION BEGINS SIMULATION MATRICES FOR PLAYER ON SECOND NO ONE OUT


# PROVIDES THE PROBABILITY OF SCORING ONE RUN FROM A BASE-OUT-STATE ->
# PROVIDES THE TRANSITION PROBABILITY OF MOVING FROM ONE STATE TO MAN ON SECOND NO ONE OUT ->
# PROVIDES PROBABILITY OF A STATE OCCURRING AFTER MAN ON SECOND NO ONE OUT
for(i in 1:length(t2)){
  ...
  assign(paste0(t2[i], "_2Bpmatrix"), cbind(get(paste0(t2[i], "_prob1")), get(paste0(t2[i], "_2Bpmatrix"))))
  ...
  assign(paste0(t2[i], "_2Bpmatrix"), dplyr::mutate(get(paste0(t2[i], "_2Bpmatrix")),
                                                    combined_prob = case_when(transfer_prob > 0 ~ run_prob * run_prob[7],
                                   TRUE ~ run_prob * transfer_prob)))
}

##----
# GENERATE TRANSITION MATRIX FOR ONLY 010 0 STATE ->
# TO BE USED WITH SIMULATION FUNCTIONS
for(i in 1:length(t3)){
  assign(paste0(t2[i], "_2Btmatrix"), 
         dplyr::select(get(t4[i]), 
                       state, 
                       new_state) %>%
...
##----
# GENEREATE PROBABILITY TABLES FROM 2B TRANSITION MATRICES
